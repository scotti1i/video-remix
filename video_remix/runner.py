...
def prepare(root, ids, provider):
    ...
    for variant_id in ids:
        ...
        items.append((directory, spec, assets, run, provider.arguments(spec, root, assets)))
...
def execute(root, ids, budget, estimate, execute=False, wait=0, provider=None):
    ...
    with locked(root / ".project.lock"):
        items = prepare(root, ids, provider)
