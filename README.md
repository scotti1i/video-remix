...
当前稳定版本 `0.3.9`。不提供零密钥演示；分析和生成使用用户自己的账号。流程依据真实实验改进，**不保证任意商品一次成功，也未证明批量成功率**。Gemini 分析、宿主垫图生成与即梦原视频产物是不同节点。机器审片仅在测试时辅助定位，不是生产必经门，不能代替客户判断。开发候选提供分镜独立生成＋明确区间硬切组装，见下文；不代表稳定版或旧项目已获得此能力。
