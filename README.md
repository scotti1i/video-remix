...
## 维护边界：节点通过文件交接，不通过会话记忆

|节点|输入→持久输出|职责边界|
|---|---|---|
|观察（analysis）|登记素材→原始响应、观察文本、局部校正|记录源事实；创意取舍不混成事实校正|
|制作（Skill）|用户目标＋原观察/校正＋商品/画面素材→逐镜plan|决定保留和替换；关键源事实指向实际动作/声音/图片，不能只存在摘要|
|装配/裂变（creative、variation）|plan/显式替换→冻结plan、spec、diff|原文装配，不重新分析或写词；不保证语义覆盖|
|运行（runner、dreamina）|spec＋真实素材→平台ID、费用、原件|供应商接口与执行状态分开；恢复原ID，不按审片分数重抽|
|直出/组装（production_job、assembly）|已登记镜头＋组装合同→可恢复状态、成片|只编排和组装，不决定爆点，不隐式改变音轨或切点|
|诊断（assess、独立检查）|源事实＋实际输入/输出→问题证据与建议|只定位哪一层偏离；不能回写已提交任务或伪造客户选择|

定位从最早偏离的交接物开始：源没看对查观察；源有但正文无查制作；正文有但规格/参数不同查装配；已如实提交仍未执行才测试输入表达或模型。当前观察文本与`provenance`是人/Agent可读证据，不是自动语义验证合同；不要用非空字段或软件测试声称“全信息保真”。

程序版本与内容项目分离：更新默认后端不改旧项目锁；升级显式检查当前已有节点格式并备份元数据，不复制媒体。新节点引入持久记录时同步补兼容检查、备份、在途状态/锁和旧项目回归；改变已有数据含义才升级格式，不靠另写一份状态摘要迁移。实验计划/私人素材留用户项目，通用决策修订合并进对应Skill，确定性逻辑用离线测试；不把每个品类写成后端分支。
...
同时补齐已有计划、组装、直出和整片裂变的升级兼容检查与元数据备份，备份包含分析正文和局部校正；未知/未结束的编排继续固定原版本。升级复用既有编排锁，避免直接执行入口与切版并发。返回工作指导的真实目录，兼容旧名称但不让相对链接指向不存在的文件。没有新迁移框架或自动语义验收。
