...
本地候选版本 `0.4.0rc5`；已发布稳定版本仍为 `0.3.9`，不因本机更新自动发布公共仓库。不提供零密钥演示；分析和生成使用用户自己的账号。流程依据真实实验改进，**不保证任意商品一次成功，也未证明批量成功率**。Gemini 分析、宿主垫图生成与即梦原视频产物是不同节点。机器审片仅在测试时辅助定位，不是生产必经门，不能代替客户判断。候选支持分镜独立生成、明确区间硬切组装和按镜头派生整片裂变；旧项目仍固定旧版，不自动迁移。分析接口明确截断或被阻断时保留证据、标为不完整，不继续当作成功输入或自动付费重试。
...
候选新增 `produce <production.json>`：Skill 先从真实参考确定拟保留机制，完成换人/换景的逐镜图片与计划；后端将已登记镜头的串行生成、原任务恢复、下载、组装接成同一入口。默认预览，`--execute --wait 30` 执行，未完成恢复同一合同。独立 `video-remix-production.v1` 不改变旧格式；不自动写词、生图、选片或重抽。见[裂变直出](skills/sct-video-remix/references/direct-production.md)。代码验证使用假平台与真实本地视频；它不证明生成质量。

0.4.0rc3 同时修复付款前检查：查询后费用变化纳入后续预算、整批已知失败/不明先阻断新单、已下载原片真实时长/音轨/画幅不满足时不继续为余镜付款。已有付费任务仍按原ID收取。新增费用未知显示 `null`，不以预估或零冒充实付。

0.4.0rc4 将 `produce --execute` 回执收紧为状态、任务ID、费用和输出路径，完整提示词及媒体探测仍保留在返回的run文件；主控恢复不再反复接收整份证据。只读预览保持完整。本项改变信息交接，不代表画质改善，也不改变既有项目锁。

0.4.0rc5 将源事实覆盖检查前移到制作准备：局部校正不替换整份分析，关键展示从源记录追到实际动作/声音/图片，制作取舍不冒充源事实。支持按已有离线入口独立复测到提交参数，不为发现文字遗漏先付费生片；无新增强制合同或生产审批，不能据此承诺模型执行效果。
