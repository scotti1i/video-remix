...
开始制作、复刻或裂变时完整读取 [制作方法](../../sct-video-remix/references/production.md)。使用 `compile`、`vary` 或 `vary-assembly` 时读取 [制作计划合同](../../sct-video-remix/references/plan-contract.md)；有口播按制作方法路由读说话指导。链接兼容新旧 Skill 路径。故障查询和单纯下载不必重新分析素材。当前工作指导与后端为本地候选 0.4.0rc5；旧项目升级前继续遵守其固定版。
...
0.3.1 起，相同素材 SHA、模型、网关、完整分析模板与 brief 的成功分析会命中缓存，返回 `cache_hit: true`，不重复发请求。需要重新观察时显式加 `--refresh`，保留旧结果；旧版没有签名的记录仍可人工读取复用，不自动猜等价。明确截断或被阻断的响应标为 `incomplete`，保留原响应、部分文本与用量，不当成功输入；旧缓存也核对结束原因，不自动重新付费。没有结束原因的旧网关结果保持兼容，但标注 `completion_verified: false`，不能称已验证完整。区间以实际登记的视频文件为准，改变区间需明确准备/登记媒体。若旁边有 `corrections.md`，缓存结果同时返回其路径，制作时与原分析一起读取。
...
开发候选增加按真实切镜独立生成与 `assemble` 组装，读[分镜生成与组装](shot-generation.md)。每镜仍使用现有 `compile/run`，没有另一个自动改写提示词节点。旧固定后端不提供组装命令，不强行调用或升级运行中项目。一镜到底超过单段上限仍需说明缺口，不能硬切后称无缝延续；也不能把单段上限当默认交付时长。
...
0.3.8 起 CLI 错误只保存白名单分类、固定原因和有界退出码/超时值，不记录可能含密钥的原始输出。上传确认超时不再混同为生成效果失败；它仍不证明未提交或未扣费，不能据此重交。新任务的 `generation_inputs` 精确保存规格中的输入角色，历史 `inputs` 素材库角色保持原义；`submit_attempts` 只记录本后端提交，不能当平台或手工操作总次数。

## 4. 管理与交付

多镜头制作按 `shot-generation.md` 保存各镜计划及源区间，全部原片下载后执行：

```sh
--project <目录> assemble /absolute/assembly.json
...
开发候选新增 `inventory --root <项目集合>`，只读汇总一个项目或一层项目目录的版本、任务、费用与本地原片存在性，不调用生成平台、不修改版本锁。用于跨批复盘和核对交付遗漏，不把 `downloaded` 当客户验收。旧固定后端没有此命令时，不为汇总升级项目；显式使用支持它的开发后端。实际状态、异常、重复任务与费用未知分别保留，不用文件数量冒充独立生成次数。

先交付视频，再写简短结论。运行成功仅表示文件生成，不表示商品效果或自然度合格。

测试迭代或用户提出声音问题时，可选做一次目标明确的音轨对比（使用 Gemini 分析额度，不是即梦生成积分）；不是生产必经门，不因未做它扣住交付，也不代表客户接受：
...
0.3.8 起，无独立计划的同条件 `rerun` 可沿父链继承最近计划的诊断目标，逐跳核对生成条件和已有提交摘要；不跨越没有自身计划的 `variation`，不猜测缺失或不匹配目标。诊断不能替代客户判断，不自动选片或触发重生成。
