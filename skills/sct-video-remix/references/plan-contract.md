...
`compile /absolute/plan.json` 逐字装配 prompt 并登记一个版本，不运行外部模型。正文只写希望模型执行的指令；分析来源、推测、替换说明放 `provenance`，它们随计划保存但不混进 prompt。“本次离线测试、待审核、临时占位、素材尚缺”等工作状态也留在来源/交接记录，不写进 `sound/action` 让生成模型解释制作流程。未准备好的输入如实阻止该次实际提交，不以编译通过掩盖缺口。
...
可选 `generation_route` 指实际平台入口，和 `mode` 分开：省略或 `reference` 沿用多素材参考；显式 `first_frame` 使用原生首帧入口。首帧路线仅接一张图片、不能额外传商品图或音视频，实际图片画幅须符合计划；提示词里不能再引用未提供的辅助图。平台从图片推断画幅，后端不传 `--ratio`。当前首帧适配范围仅 Seedance 2.0 标准/fast 及各自 VIP 型号，具体模型仍遵守用户选择；帮助中的其他型号不因此自动开放。只在固定运行时支持该路线时使用，按对应命令帮助预检；旧项目不会因默认后端升级自动改变路线。改变入口不是同条件重生，使用关联的 variation 并说明输入取舍。续段的制作选择及限制见[分镜指导](shot-generation.md#连续长镜头的显式续段路线)。

`shots` 用输出时间，`provenance.source_interval` 用原片时间。允许一镜连续变化或多镜，不强制三拍；镜头有序、不重叠、不超输出时长。声音可跨切镜延续，在 sound 或 sync 说明。

多镜头独立生成时，每个生成任务各写一份本合同，本地时间从0开始，`provenance.source_interval` 记录对应原镜头。共有商品/人物/场景约束保持一致，输入只选本镜需要的素材。任务之间的顺序、取用区间和全片声音由独立组装合同管理，见[分镜生成与组装](shot-generation.md)；不要把整片所有镜头再次塞入各段prompt。若已选择独立连续旁白，本镜 `sound` 明确只生成画面及所需现场声、不生成旁白；全文留在连续声音制作记录，`provenance.audio_change` 指向其来源和组装安排。`sync` 仍说明相应动作时段，但不让未收到音轨的模型假装按音频同步；最终用真实声画核对。原v1合同语义不变。
...
没有独立商品图片输入的镜头（例如只输入透镜清态/暗态的近距镜头），可显式填写 `frozen_core.product_assets: []`，再分别替换两张该镜画面图；不要为满足字段强塞会冲突的全貌商品照。空列表只表示本次没有受保护的商品图片 ID，不是商品一致性证据，也不放开 `product/intent/sound` 等文字或图片数量、类型的修改。已有独立商品真源仍应列入非空列表，它及其角色继续禁止替换；后端不根据图片内容猜谁是商品真源。单镜 `vary` 与整片 `vary-assembly` 使用相同语义。这个兼容扩展无需改旧控制文件，但旧固定后端若拒绝空列表，应采用支持它的候选运行时，不修改运行中的项目。
...
多镜头成片使用开发候选入口 `vary-assembly /absolute/film-control.json`，默认只读预览；加 `--execute` 才登记新镜头计划和新组装合同，不调用外部模型或渲染。它复用上面的单镜 `variation` 控制，不再次改写提示词，不要求父镜头已下载或经过机器选片。

```json
{
  "format": "video-remix-assembly-variation.v1",
  "id": "new-scene-cut",
  "parent": "/absolute/project/assemblies/base-cut/contract.json",
  "changes": [
    {
      "clip": 2,
      "control": {
        "format": "video-remix-variation.v1",
        "id": "new-scene-shot-02",
        "parent": "shot-02",
        "frozen_core": {"mechanism": "保留镜内变暗、恢复和再次触发。", "product_assets": ["product"]},
        "slots": {"scene": {
          "text": [{"path": "look", "from": "blue door", "to": "green door"}],
          "images": [{"from": "shot-02-visual", "to": "shot-02-new-visual", "role": "本镜新场景透视画面，非强制首帧"}]
        }}
      }
    },
    {"clip": 4, "variant": "shot-04-repaired"}
  ]
}
```

`parent` 是明确的 v1/v2 组装合同文件；相对路径以本控制文件目录为基准，已有冻结组装必须与它相符。`clip` 从 **1** 起，对应父合同的列表位置，同一父版本在多处出现也不会含糊；`changes` 只列要换的镜头，每镜恰选完整 `control` 或已登记 `variant`。新素材先登记；旧父计划与冻结规格须一致。已有返修版本必须直接关联该镜父版本，并保留提供方、模型、时长、画幅、分辨率和输入模式；这是显式接入已准备的返修，不声称其提示词其他部分未经改动。

新合同只替换指定位置的版本 ID，逐项保留 `in/out`、镜头顺序和完整 `audio` 对象，未改镜头沿用原片，不自动全部重生。连续声音仍引用原指定音轨，不随画面版本自动更换；`clips` 仍取各镜自己的声音，因此换镜也会换该镜原声，不能宣称声线保持不变。需要改声音或剪法时另建明确合同，不把它藏进此窄入口。

完整预检后才登记，结果在 `assembly-variations/<id>/`，包含父合同快照、控制、逐镜差异和新 `assembly.json`。后续仅生成新增版本，下载后执行该合同的 `assemble`。重复批次/组装 ID 拒绝覆盖。若登记中断，保留已登记镜头及 `run.json`；核对 `plans/variants` 后用新批次 ID，完整子镜头用 `variant` 引用，未完整登记的子镜头换新 ID，不自动重抽或抹掉失败记录。它冻结字段，不保证跨镜身份一致、商品机构或核心卖点已经正确呈现。
