...
多镜头独立生成时，每个生成任务各写一份本合同，本地时间从0开始，`provenance.source_interval` 记录对应原镜头。共有商品/人物/场景约束保持一致，输入只选本镜需要的素材。任务之间的顺序、取用区间和全片声音由独立组装合同管理，见[分镜生成与组装](shot-generation.md)；不要把整片所有镜头再次塞入各段prompt。原v1合同语义不变。
